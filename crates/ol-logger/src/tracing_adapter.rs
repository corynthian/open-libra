...
/// A layer that translates tracing events into ol-logger events.
pub struct TracingToOLDataLayer;
...
impl<S> Layer<S> for TracingToOLDataLayer
where
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    fn on_event(&self, event: &Event, ctx: Context<S>) {
        let metadata = match translate_metadata(event.metadata()) {
            ...
            None => {
                dl::warn!(
                    "[tracing-to-ol-logger] failed to translate event due to unknown level {:?}",
