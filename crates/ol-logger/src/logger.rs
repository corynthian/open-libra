...
pub fn set_global_logger(logger: Arc<dyn Logger>, console_port: Option<u16>) {
     ...
     * else ol-logger intercepts all tracing::log events
     * In both scenarios *ALL* ol-logger::log events are captured by ol-logger as usual.
     */
    #[cfg(feature = "ol-console")]
    {
    ...
    if console_port.is_none() {
        let _ = tracing::subscriber::set_global_default(
            crate::tracing_adapter::TracingToOLDataLayer
                .with_subscriber(tracing_subscriber::Registry::default()),
        );
    } else {
        error!("console_port was set but has no effect, build with --cfg ol-console");
    }
}
