...
/// A builder for a `OLData`, configures what, where, and how to write logs.
pub struct OLDataBuilder {
    channel_size: usize,
...
impl OLDataBuilder {
    ...
    fn build_logger(&mut self) -> Arc<OLData> {
    ...
    pub fn build(&mut self) -> Arc<OLData> {
        let logger = self.build_logger();

        let console_port = if cfg!(feature = "ol-console") {
            self.console_port
        } else {
            None
        };
...
impl OLData {
    pub fn builder() -> OLDataBuilder {
        OLDataBuilder::new()
    }

    #[allow(clippy::new_ret_no_self)]
    pub fn new() -> OLDataBuilder {
        Self::builder()
    }

    pub fn init_for_testing() {
        // Create the OL Data Builder
...
/// or to a `OLData` for external logging.
...
/// Example:
/// 2020-03-07 05:03:03 INFO [thread_name] common/ol-logger/src/lib.rs:261 Hello { "world": true }
...
pub struct LoggerFilterUpdater {
    logger: Arc<OLData>,
    logger_builder: OLDataBuilder,
}

impl LoggerFilterUpdater {
    pub fn new(logger: Arc<OLData>, logger_builder: OLDataBuilder) -> Self {
        Self {
            logger,
            logger_builder,
        }
...
#[cfg(test)]
mod tests {
    use super::{OLData, LogEntry};
    use crate::{
        ol_logger::{json_format, RUST_LOG_TELEMETRY},
        debug, error, info,
        logger::Logger,
        trace, warn, OLDataBuilder, Event, Key, KeyValue, Level, LoggerFilterUpdater, Metadata,
    ...
    fn basic() {
        ...
        let expected = format!("{{\"level\":\"INFO\",\"source\":{{\"package\":\"ol_logger\",\"file\":\"crates/ol-logger/src/ol_logger.rs:{line_num}\"}},\"thread_name\":\"{thread_name}\",\"hostname\":\"test-host\",\"timestamp\":\"2022-07-24T23:42:29.540278Z\",\"message\":\"This is a log\",\"data\":{{\"bar\":\"foo_bar\",\"category\":\"name\",\"display\":\"12345\",\"foo\":5,\"test\":true}}}}");
    ...
    fn new_async_logger() -> (OLDataBuilder, Arc<OLData>) {
        let mut logger_builder = OLDataBuilder::new();
