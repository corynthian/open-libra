...
use crate::{
    ...
    test_utils::{random_subset, KeyPair, TestOLCrypto},
...
fn random_message_for_signing(rng: &mut OsRng) -> TestOLCrypto {
    TestOLCrypto(
...
fn random_messages_for_signing(rng: &mut OsRng, n: usize) -> Vec<TestOLCrypto> {
    (0..n)
        .map(|_| random_message_for_signing(rng))
        .collect::<Vec<TestOLCrypto>>()
}
...
fn bls12381_aggsig_should_verify() {
    ...
    let msgs_refs = messages.iter().collect::<Vec<&TestOLCrypto>>();
    ...
    let msgs_wrong_refs = messages_wrong.iter().collect::<Vec<&TestOLCrypto>>();
...
/// language/ol-framework/move-stdlib/sources/signer.move
...
fn bls12381_sample_signature_verifies() {
    ...
    assert!(sig.verify_arbitrary_msg(b"Hello OL!", &pk).is_ok());
}
...
fn bls12381_sample_doc_test_for_normal_sigs() {
    ...
    println!("// signature on TestOLCrypto(\"test\".to_owned())");
    println!(
...
fn bls12381_sample_aggregate_pk_and_aggsig() {
    ...
    println!(
        "// The signed messages are \"Hello, OL <i>!\", where <i> \\in {{1, ..., {}}}",
        num
    );
