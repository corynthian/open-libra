...
) -> impl Strategy<Value = (EdwardsPoint, EdwardsPoint, TestOLCrypto)> {
    (
...
pub struct TestOLCrypto(pub String);
...
// #[cfg(any(test, feature = "fuzzing"))]
pub struct TestOLCryptoHasher(crate::hash::DefaultHasher);
// #[cfg(any(test, feature = "fuzzing"))]
impl ::core::clone::Clone for TestOLCryptoHasher {
    #[inline]
    fn clone(&self) -> TestOLCryptoHasher {
        match *self {
            TestOLCryptoHasher(ref __self_0_0) => {
                TestOLCryptoHasher(::core::clone::Clone::clone(__self_0_0))
            },
        }
    }
}
...
// #[cfg(any(test, feature = "fuzzing"))]
impl TestOLCryptoHasher {
    fn new() -> Self {
        let name = crate::_serde_name::trace_name::<TestOLCrypto>()
            .expect("The `CryptoHasher` macro only applies to structs and enums");
        TestOLCryptoHasher(crate::hash::DefaultHasher::new(name.as_bytes()))
    }
}
// #[cfg(any(test, feature = "fuzzing"))]
static TEST_CRYPTO_HASHER: crate::_once_cell::sync::Lazy<TestOLCryptoHasher> =
    crate::_once_cell::sync::Lazy::new(TestOLCryptoHasher::new);
// #[cfg(any(test, feature = "fuzzing"))]
impl std::default::Default for TestOLCryptoHasher {
    fn default() -> Self {
        TEST_CRYPTO_HASHER.clone()
    }
}
// #[cfg(any(test, feature = "fuzzing"))]
impl crate::hash::CryptoHasher for TestOLCryptoHasher {
...
// #[cfg(any(test, feature = "fuzzing"))]
impl std::io::Write for TestOLCryptoHasher {
...
// #[cfg(any(test, feature = "fuzzing"))]
impl crate::hash::CryptoHash for TestOLCrypto {
    type Hasher = TestOLCryptoHasher;

    fn hash(&self) -> crate::hash::HashValue {
        use crate::hash::CryptoHasher;
        let mut state = Self::Hasher::default();
        bcs::serialize_into(&mut state, &self)
            .expect("BCS serialization of TestOLCrypto should not fail");
        state.finish()
    }
}

/// Produces a random TestOLCrypto signable / verifiable struct.
#[cfg(any(test, feature = "fuzzing"))]
pub fn random_serializable_struct() -> impl Strategy<Value = TestOLCrypto> {
    (String::arbitrary()).prop_map(TestOLCrypto).no_shrink()
}
