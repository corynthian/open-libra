...
use crate::{
    ...
    ol_vm_impl::{get_transaction_output, OLVMImpl, OLVMInternals},
    block_executor::BlockOLVM,
    counters::*,
...
use ol_crypto::HashValue;
use ol_framework::natives::code::PublishRequest;
use ol_gas::{OLGasMeter, ChangeSetConfigs};
...
#[derive(Clone)]
pub struct OLVM(pub(crate) OLVMImpl);

struct OLSimulationVM(OLVM);

impl OLVM {
    pub fn new<S: StateView>(state: &S) -> Self {
        Self(OLVMImpl::new(state))
    }
    ...
    pub fn internals(&self) -> OLVMInternals {
        OLVMInternals::new(&self.0)
    }
    ...
    fn validate_publish_request<S: MoveResolverExt>(
        ...
        for m in modules {
            ...
            ol_framework::verify_module_metadata(m, self.0.get_features())
    ...
    fn validate_waypoint_change_set(
        ...
        if has_new_block_event && has_new_epoch_event {
            ...
            error!(
                *log_context,
                "[ol_vm] waypoint txn needs to emit new epoch and block"
            );
    ...
    pub(crate) fn process_waypoint_change_set<S: MoveResolverExt + StateView>(
        ...
        let change_set_ext = match self.execute_writeset(
            storage,
            &writeset_payload,
            Some(ol_types::account_config::reserved_vm_address()),
    ...
    pub fn simulate_signed_transaction(
        ...
        let vm = OLVM::new(state_view);
        let simulation_vm = OLSimulationVM(vm);
...
impl VMExecutor for OLVM {
    ...
    fn execute_block(
        ...
        let ret =
            BlockOLVM::execute_block(transactions, state_view, Self::get_concurrency_level());
        if ret.is_ok() {
...
impl VMAdapter for OLVM {
    ...
    fn should_restart_execution(vm_output: &TransactionOutput) -> bool {
        let new_epoch_event_key = ol_types::on_chain_config::new_epoch_event_key();
        vm_output
            .events()
            .iter()
    ...
    fn execute_single_transaction<S: MoveResolverExt + StateView>(
        ...
        Ok(match txn {
            PreprocessedTransaction::BlockMetadata(block_metadata) => {
                fail_point!("ol_vm::execution::block_metadata");
            ...
            PreprocessedTransaction::UserTransaction(txn) => {
                fail_point!("ol_vm::execution::user_transaction");
                    ...
                    vm_status.clone().keep_or_discard()
                {
                    error!(
                        *log_context,
                        "[ol_vm] Transaction breaking invariant violation. txn: {:?}",
...
impl AsRef<OLVMImpl> for OLVM {
    fn as_ref(&self) -> &OLVMImpl {
        &self.0
    }
}

impl AsMut<OLVMImpl> for OLVM {
    fn as_mut(&mut self) -> &mut OLVMImpl {
        &mut self.0
    }
}
