...
use ol_aggregator::transaction::TransactionOutputExt;
use ol_framework::RuntimeModuleMetadataV1;
use ol_gas::{
    AbstractValueSizeGasParameters, OLGasParameters, ChangeSetConfigs, FromOnChainGasSchedule,
    Gas, NativeGasParameters, StorageGasParameters,
};
use ol_logger::prelude::*;
use ol_state_view::StateView;
use ol_types::{
    account_config::{TransactionValidation, OL_TRANSACTION_VALIDATION, CORE_CODE_ADDRESS},
    chain_id::ChainId,
    on_chain_config::{
...
pub struct OLVMImpl {
    ...
    gas_params: Option<OLGasParameters>,
...
impl OLVMImpl {
    ...
    pub fn new<S: StateView>(state: &S) -> Self {
        ...
        let (mut gas_params, gas_feature_version): (Option<OLGasParameters>, u64) =
            match GasScheduleV2::fetch_config(&storage) {
                Some(gas_schedule) => {
                    let feature_version = gas_schedule.feature_version;
                    let map = gas_schedule.to_btree_map();
                    (
                        OLGasParameters::from_on_chain_gas_schedule(&map, feature_version),
                        feature_version,
                    )
                },
                None => match GasSchedule::fetch_config(&storage) {
                    Some(gas_schedule) => {
                        let map = gas_schedule.to_btree_map();
                        (OLGasParameters::from_on_chain_gas_schedule(&map, 0), 0)
                    },
                    None => (None, 0),
                },
            };
    ...
    pub(crate) fn transaction_validation(&self) -> &TransactionValidation {
        self.transaction_validation
            .as_ref()
            .unwrap_or(&OL_TRANSACTION_VALIDATION)
    }
    ...
    ) -> Result<&OLGasParameters, VMStatus> {
    ...
    pub(crate) fn extract_module_metadata(
        ...
        if self.features.is_enabled(FeatureFlag::VM_BINARY_FORMAT_V6) {
            ol_framework::get_vm_metadata(&self.move_vm, module.clone())
        } else {
            ol_framework::get_vm_metadata_v0(&self.move_vm, module.clone())
        }
    }
...
#[derive(Clone, Copy)]
pub struct OLVMInternals<'a>(&'a OLVMImpl);

impl<'a> OLVMInternals<'a> {
    pub fn new(internal: &'a OLVMImpl) -> Self {
        Self(internal)
    }
    ...
    pub fn gas_params(
        self,
        log_context: &AdapterLogSchema,
    ) -> Result<&'a OLGasParameters, VMStatus> {
