...
use ol_framework::{ResourceGroupScope, RuntimeModuleMetadataV1};
...
pub(crate) fn validate_module_and_extract_new_entries<S: MoveResolverExt>(
    ...
    let (new_groups, mut new_members) =
        if let Some(metadata) = ol_framework::get_module_metadata(module) {
    ...
    let metadata = session
        ...
        .map(|module| {
            CompiledModule::deserialize(&module)
                .map(|module| ol_framework::get_module_metadata(&module))
        });
