...
use crate::{
    ...
    block_executor::vm_wrapper::OLExecutorTask,
    counters::{
...
use ol_aggregator::{delta_change_set::DeltaOp, transaction::TransactionOutputExt};
use ol_block_executor::{
    errors::Error,
...
pub(crate) struct OLTransactionOutput(TransactionOutputExt);

impl OLTransactionOutput {
...
impl BlockExecutorTransactionOutput for OLTransactionOutput {
        ...
        let signature_verified_block: Vec<PreprocessedTransaction> =
            RAYON_EXEC_POOL.install(|| {
                transactions
                    .into_par_iter()
                    .with_min_len(25)
                    .map(preprocess_transaction::<OLVM>)
                    .collect()
            });
        drop(signature_verification_timer);

        BLOCK_EXECUTOR_CONCURRENCY.set(concurrency_level as i64);
        let executor = BlockExecutor::<PreprocessedTransaction, OLExecutorTask<S>, S>::new(
            concurrency_level,
        );
