...
use ol_types::{on_chain_config::Features, transaction::AbortInfo};
...
/// 0L specific metadata (`ol::` here).
pub static OL_METADATA_KEY: Lazy<Vec<u8>> =
    Lazy::new(|| "ol::metadata_v0".as_bytes().to_vec());
pub static OL_METADATA_KEY_V1: Lazy<Vec<u8>> =
    Lazy::new(|| "ol::metadata_v1".as_bytes().to_vec());

/// 0L specific metadata attached to the metadata section of file_format.
...
/// V1 of 0L specific metadata attached to the metadata section of file_format.
...
pub fn get_vm_metadata(vm: &MoveVM, module_id: ModuleId) -> Option<RuntimeModuleMetadataV1> {
    if let Some(data) = vm.get_module_metadata(module_id.clone(), &OL_METADATA_KEY_V1) {
        bcs::from_bytes::<RuntimeModuleMetadataV1>(&data.value).ok()
    } else if let Some(data) = vm.get_module_metadata(module_id, &OL_METADATA_KEY) {
...
pub fn get_vm_metadata_v0(vm: &MoveVM, module_id: ModuleId) -> Option<RuntimeModuleMetadataV1> {
    if let Some(data) = vm.get_module_metadata(module_id, &OL_METADATA_KEY) {
        let data_v0 = bcs::from_bytes::<RuntimeModuleMetadata>(&data.value).ok()?;
        Some(data_v0.upgrade())
    } else {
        None
...
pub fn get_module_metadata(module: &CompiledModule) -> Option<RuntimeModuleMetadataV1> {
    if let Some(data) = find_metadata(module, &OL_METADATA_KEY_V1) {
        ...
        metadata
    } else if let Some(data) = find_metadata(module, &OL_METADATA_KEY) {
        // Old format available, upgrade to new one on the fly
        let data_v0 = bcs::from_bytes::<RuntimeModuleMetadata>(&data.value).ok()?;
        Some(data_v0.upgrade())
    } else {
