...
module std::bls12381 {
    ...
    /// Random signature generated by running `cargo test -- bls12381_sample_signature --nocapture --include-ignored` in `crates/ol-crypto`.
    /// The message signed is "Hello OL!" and the associated SK is 07416693b6b32c84abe45578728e2379f525729e5b94762435a31e65ecc728da.
    ...
    fun test_pubkey_aggregation() {
        // Second, try some test-cases generated by running the following command in `crates/ol-crypto`:
    ...
    fun test_verify_aggsig() {
        ...
        // The signed messages are "Hello, OL <i>!", where <i> \in {1, ..., 5}
        let msgs = vector[
    ...
    fun test_verify_aggregated_signature_randomized() {
        ...
        while (signer_count <= max_signer_count) {
            ...
            while (i < signer_count) {
                ...
                std::vector::push_back(&mut signing_keys, sk);
                std::vector::push_back(&mut public_keys, pk);
                let msg: vector<u8> = vector[104, 101, 108, 108, 111, 32, 97, 112, 116, 111, 115, 32, 117, 115, 101, 114, 32, 48+(i as u8)]; //"hello ol user {i}"
    ...
    fun test_verify_pop() {
        // Test case generated by running `cargo test -- sample_pop --nocapture --include-ignored` in `crates/ol-crypto`
        // =============================================================================================================
