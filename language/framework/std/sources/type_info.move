module std::type_info {
    ...
    /// Returns the current chain ID, mirroring what `ol_framework::chain_id::get()` would return, except in `#[test]`
    /// functions, where this will always return `4u8` as the chain ID, whereas `ol_framework::chain_id::get()` will
    /// return whichever ID was passed to `ol_framework::chain_id::initialize_for_test()`.
    public fun chain_id(): u8 {
        if (!features::std_chain_id_enabled()) {
    ...
    fun test() {
        ...
        assert!(account_address(&type_info) == @std, 0);
    ...
    fun test_chain_id(fx: signer) {
        ...
        features::change_feature_flags(&fx, vector[features::get_std_chain_id_feature()], vector[]);
