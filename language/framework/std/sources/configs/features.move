...
module std::features {
    ...
    /// Whether the new `std::type_info::chain_id()` native for fetching the chain ID is enabled.
    /// This is needed because of the introduction of a new native function.
    /// Lifetime: transient
    const STD_CHAIN_ID_NATIVES: u64 = 4;

    public fun get_std_chain_id_feature(): u64 { STD_CHAIN_ID_NATIVES }

    public fun std_chain_id_enabled(): bool acquires Features {
        is_enabled(STD_CHAIN_ID_NATIVES)
    }
    ...
    /// Whether the new `std::multi_ed25519::public_key_validate_internal_v2()` native is enabled.
    /// This is needed because of the introduction of a new native function.
    /// Lifetime: transient
