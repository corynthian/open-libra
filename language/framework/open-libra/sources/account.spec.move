spec open_libra::account {
    spec module {
        pragma verify = true;
        pragma aborts_if_is_strict;
    }

    /// Only the address `@open_libra` can call.
    /// OriginatingAddress does not exist under `@open_libra` before the call.
    spec initialize(open_libra: &signer) {
        ...
        aborts_if exists<OriginatingAddress>(ol_addr);
        ensures exists<OriginatingAddress>(ol_addr);
    }
    ...
    spec create_account(new_address: address): signer {
        ...
        aborts_if new_address == @vm_reserved || new_address == @open_libra;
    ...
    spec rotate_authentication_key(
        ...
        modifies global<Account>(addr);
        modifies global<OriginatingAddress>(@open_libra);
    }
