module open_libra::transaction_validation {
    use std::error;
    use std::features;
    use std::signer;
    use std::vector;
    ...
    public(friend) fun initialize(
        ...
        move_to(open_libra, TransactionValidation {
    ...
    fun prologue_common(
        ...
        assert!(
            coin::is_account_registered<OLCoin>(transaction_sender),
            error::invalid_argument(PROLOGUE_ECANT_PAY_GAS_DEPOSIT),
        );
        let balance = coin::balance<OLCoin>(transaction_sender);
    ...
    fun epilogue(
        ...
        assert!(
            coin::balance<OLCoin>(addr) >= transaction_fee_amount,
