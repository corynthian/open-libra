...
module open_libra::coin {
    ...
    use open_libra::aggregator::{Self, Aggregator};
    use open_libra::event::{Self, EventHandle};
    use open_libra::optional_aggregator::{Self, OptionalAggregator};
    use open_libra::system_addresses;

    friend open_libra::ol_coin;
    ...
    public(friend) fun initialize_supply_config(open_libra: &signer) {
        system_addresses::assert_open_libra(open_libra);
        move_to(open_libra, SupplyConfig { allow_upgrades: false });
    }
    ...
    public(friend) fun initialize_aggregatable_coin<CoinType>(open_libra: &signer): AggregatableCoin<CoinType> {
        let aggregator = aggregator_factory::create_aggregator(open_libra, MAX_U64);
    ...
    public entry fun upgrade_supply<CoinType>(account: &signer) acquires CoinInfo, SupplyConfig {
        ...
        assert!(
            borrow_global_mut<SupplyConfig>(@open_libra).allow_upgrades,
    ...
    #[test(source = @0x2, framework = @open_libra)]
    ...
    #[test(framework = @open_libra, other = @0x123)]
    ...
    #[test(framework = @open_libra)]
    #[expected_failure(abort_code = 0x20001, location = open_libra::aggregator)]
    ...
    #[test(framework = @open_libra)]
    #[expected_failure(abort_code = 0x5000B, location = open_libra::coin)]
