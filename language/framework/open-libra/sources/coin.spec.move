spec open_libra::coin {
    ...
    spec initialize_supply_config(open_libra: &signer) {
        let ol_addr = signer::address_of(open_libra);
        aborts_if !system_addresses::is_open_libra_address(ol_addr);
        aborts_if exists<SupplyConfig>(ol_addr);
        ensures !global<SupplyConfig>(ol_addr).allow_upgrades;
        ensures exists<SupplyConfig>(ol_addr);
    }

    /// Can only be updated by `@open_libra`.
    spec allow_supply_upgrades(open_libra: &signer, allowed: bool) {
        modifies global<SupplyConfig>(@open_libra);
        let ol_addr = signer::address_of(open_libra);
        aborts_if !system_addresses::is_open_libra_address(ol_addr);
        aborts_if !exists<SupplyConfig>(ol_addr);
        let post allow_upgrades_post = global<SupplyConfig>(@open_libra);
    ...
    /// The creator of `CoinType` must be `@open_libra`.
    ...
    spec upgrade_supply<CoinType>(account: &signer) {
        ...
        aborts_if !exists<SupplyConfig>(@open_libra);
        aborts_if !exists<CoinInfo<CoinType>>(account_addr);
        let supply_config = global<SupplyConfig>(@open_libra);
    ...
    spec initialize_aggregatable_coin<CoinType>(open_libra: &signer): AggregatableCoin<CoinType> {
        // TODO: specify this.
    }
