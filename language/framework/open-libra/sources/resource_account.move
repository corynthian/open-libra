...
/// }
/// ```
module open_libra::resource_account {
    use std::error;
    use std::signer;
    use std::vector;
    ...
    public entry fun create_resource_account_and_fund(
        ...
        let (resource, resource_signer_cap) = account::create_resource_account(origin, seed);
        coin::register<OLCoin>(&resource);
        coin::transfer<OLCoin>(origin, signer::address_of(&resource), fund_amount);
        rotate_account_authentication_key_and_store_capability(
            origin,
            resource,
            resource_signer_cap,
    ...
    public entry fun create_resource_account_and_publish_package(
        ...
        let (resource, resource_signer_cap) = account::create_resource_account(origin, seed);
        open_libra::code::publish_package_txn(&resource, metadata_serialized, code);
        rotate_account_authentication_key_and_store_capability(
            origin,
            resource,
            resource_signer_cap,
    ...
    public entry fun test_create_account_and_retrieve_cap(user: signer) acquires Container {
        ...
        let resource_addr = open_libra::account::create_resource_address(&user_addr, seed);
    ...
    #[test(user = @0x1111)]
    #[expected_failure(abort_code = 0x10002, location = std::simple_map)]
    ...
    public entry fun with_coin(framework: signer, user: signer) acquires Container {
        let user_addr = signer::address_of(&user);
        open_libra::ol_account::create_account(copy user_addr);

        let (burn, mint) = open_libra::ol_coin::initialize_for_test(&framework);
        let coin = coin::mint<OLCoin>(100, &mint);
        ...
        let resource_addr = open_libra::account::create_resource_address(&user_addr, seed);
        coin::transfer<OLCoin>(&user, resource_addr, 10);
    ...
    #[expected_failure(abort_code = 0x60005, location = open_libra::coin)]
    public entry fun without_coin(framework: signer, user: signer) acquires Container {
        let user_addr = signer::address_of(&user);
        open_libra::ol_account::create_account(user_addr);
