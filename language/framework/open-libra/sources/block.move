...
module open_libra::block {
    ...
    public(friend) fun initialize(open_libra: &signer, epoch_interval_microsecs: u64) {
        ...
        move_to<BlockResource>(
            ...
            BlockResource {
                ...
                new_block_events: account::new_event_handle<NewBlockEvent>(open_libra),
                update_epoch_interval_events: account::new_event_handle<UpdateEpochIntervalEvent>(open_libra),
            }
        );
    }
    ...
    public fun update_epoch_interval_microsecs(
        ...
        let block_resource = borrow_global_mut<BlockResource>(@open_libra);
    ...
    public fun get_epoch_interval_secs(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@open_libra).epoch_interval / 1000000
    }
	...
	// 0L-TODO: Removed proposer index based on `stake` module
        // let proposer_index = option::none();
        // if (proposer != @vm_reserved) {
        //     proposer_index = option::some(stake::get_validator_index(proposer));
        // };

        let block_metadata_ref = borrow_global_mut<BlockResource>(@open_libra);
	...
	// 0L-TODO: Commented `stake::update_performance_statistics`
        // Performance scores have to be updated before the epoch transition as the transaction that
	// triggers the transition is the last block in the previous epoch.
    ...
    public fun get_current_block_height(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@open_libra).height
    }
    ...
    public entry fun test_update_epoch_interval(open_libra: signer) acquires BlockResource {
        account::create_account_for_test(@open_libra);
        initialize(&open_libra, 1);
        assert!(borrow_global<BlockResource>(@open_libra).epoch_interval == 1, 0);
        update_epoch_interval_microsecs(&open_libra, 2);
        assert!(borrow_global<BlockResource>(@open_libra).epoch_interval == 2, 1);
    }

    #[test(open_libra = @open_libra, account = @0x123)]
    #[expected_failure(abort_code = 0x50003, location = open_libra::system_addresses)]
