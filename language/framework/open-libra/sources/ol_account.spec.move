spec open_libra::ol_account {
    ...
    spec create_account(auth_key: address) {
        ...
        ensures exists<coin::CoinStore<OLCoin>>(auth_key);
    }
    ...
    spec schema CreateAccountAbortsIf {
        ...
        aborts_if auth_key == @vm_reserved || auth_key == @open_libra;
    }
    ...
    spec assert_account_is_registered_for_apt(addr: address) {
        aborts_if !account::exists_at(addr);
        aborts_if !coin::is_account_registered<OLCoin>(addr);
    }
