module open_libra::ol_account {
    use open_libra::account::{Self, new_event_handle};
    use open_libra::ol_coin::OLCoin;
    use open_libra::coin::{Self, Coin};
    use open_libra::create_signer::create_signer;
    use open_libra::event::{EventHandle, emit_event};
    use std::error;
    use std::signer;
    use std::vector;
    ...
    public entry fun transfer(source: &signer, to: address, amount: u64) {
        ...
        coin::transfer<OLCoin>(source, to, amount)
    }
    ...
    use std::string::utf8;
    #[test_only]
    use open_libra::account::create_account_for_test;
    ...
    public fun test_transfer(alice: &signer, core: &signer) {
        ...
        let (burn_cap, mint_cap) = open_libra::ol_coin::initialize_for_test(core);
        create_account(signer::address_of(alice));
        coin::deposit(signer::address_of(alice), coin::mint(10000, &mint_cap));
        transfer(alice, bob, 500);
        assert!(coin::balance<OLCoin>(bob) == 500, 0);
        transfer(alice, carol, 500);
        assert!(coin::balance<OLCoin>(carol) == 500, 1);
        transfer(alice, carol, 1500);
        assert!(coin::balance<OLCoin>(carol) == 2000, 2);
    ...
    public fun test_transfer_to_resource_account(alice: &signer, core: &signer) {
        ...
        assert!(!coin::is_account_registered<OLCoin>(resource_acc_addr), 0);

        let (burn_cap, mint_cap) = open_libra::ol_coin::initialize_for_test(core);
        create_account(signer::address_of(alice));
        coin::deposit(signer::address_of(alice), coin::mint(10000, &mint_cap));
        transfer(alice, resource_acc_addr, 500);
        assert!(coin::balance<OLCoin>(resource_acc_addr) == 500, 1);
    ...
    public fun test_batch_transfer(from: &signer, core: &signer, recipient_1: &signer, recipient_2: &signer) {
        ...
        assert!(coin::balance<OLCoin>(recipient_1_addr) == 100, 0);
        assert!(coin::balance<OLCoin>(recipient_2_addr) == 500, 1);
        coin::destroy_burn_cap(burn_cap);
        coin::destroy_mint_cap(mint_cap);
    }
