module open_libra::account {
    ...
    friend open_libra::coin;
    friend open_libra::genesis;
    friend open_libra::resource_account;
    ...
    /// rotated.
    ///
    /// For example, imagine a freshly-created wallet with address `a` and thus also with authentication
    /// key `a`, derived from a PK `pk_a` with corresponding SK `sk_a`.
    /// It is easy to recover such a wallet given just the secret key `sk_a`, since the PK can be
    ...
    ///
    /// However, if such a wallet rotates its authentication key to `b` derived from a different PK
    /// `pk_b` with SK `sk_b`, how would account recovery work?
    /// The recovered address would no longer be 'a'; it would be `b`, which is incorrect.
    /// This struct solves this problem by mapping the new authentication key `b` to the original
    /// address `a` and thus helping the wallet software during recovery find the correct address.
    ...
    /// source account is delegating its rotation capability to the `recipient_address`.
    /// This V2 struct adds the `chain_id` and `source_address` to the challenge message, which prevents
    ...
    #[test_only]
    /// Create signer for testing, independently of an OL-style `Account`.
    ...
    public(friend) fun initialize(open_libra: &signer) {
        ...
        move_to(open_libra, OriginatingAddress {
    ...
    fun update_auth_key_and_originating_address_table(
        ...
        let address_map = &mut borrow_global_mut<OriginatingAddress>(@open_libra).address_map;
    ...
    /// `Account::signer_capability_offer::for` to the address of the resource account. While an entity
    /// may call `create_account` to attempt to claim an account ahead of the creation of a resource
    ...
    /// `Account::signer_capability_offer::for` is none. The probability of a collision where someone
    ...
    public(friend) fun create_framework_reserved_account(addr: address): (signer, SignerCapability) {
        assert!(
            addr == @0x1 ||
                ...
                addr == @0x9 ||
                addr == @0xa,
            error::permission_denied(ENO_VALID_OPEN_LIBRA_RESERVED_ADDRESS),
        );
    ...
    fun test_create_signer_for_test() {
        assert!(signer::address_of(&create_signer_for_test(@open_libra)) == @0x1, 0);
    ...
    #[test(alice = @0xa11ce)]
    #[expected_failure(abort_code = 65537, location = open_libra::ed25519)]
    ...
    //
    #[test(bob = @0x345, framework = @open_libra)]
    ...
    #[test(bob = @0x345, charlie = @0x567, framework = @open_libra)]
