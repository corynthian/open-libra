...
module open_libra::reconfiguration {
    use std::error;
    use std::features;
    use std::signer;

    use open_libra::account;
    use open_libra::event;
    ...
    // friend open_libra::governance;
    friend open_libra::block;
    friend open_libra::consensus_config;
    friend open_libra::gas_schedule;
    friend open_libra::genesis;
    friend open_libra::version;
    ...
    public(friend) fun initialize(open_libra: &signer) {
        ...
        assert!(account::get_guid_next_creation_num(signer::address_of(open_libra)) == 2, error::invalid_state(EINVALID_GUID_FOR_EVENT));
        move_to<Configuration>(
            open_libra,
            Configuration {
                epoch: 0,
                last_reconfiguration_time: 0,
                events: account::new_event_handle<NewEpochEvent>(open_libra),
            }
        );
    }
    ...
    fun disable_reconfiguration(open_libra: &signer) {
        system_addresses::assert_open_libra(open_libra);
        assert!(reconfiguration_enabled(), error::invalid_state(ECONFIGURATION));
        move_to(open_libra, DisableReconfiguration {})
    }

    /// Private function to resume reconfiguration.
    /// This function should only be used for offline WriteSet generation purpose and should never be invoked on chain.
    fun enable_reconfiguration(open_libra: &signer) acquires DisableReconfiguration {
        system_addresses::assert_open_libra(open_libra);

        assert!(!reconfiguration_enabled(), error::invalid_state(ECONFIGURATION));
        DisableReconfiguration {} = move_from<DisableReconfiguration>(signer::address_of(open_libra));
    }

    fun reconfiguration_enabled(): bool {
        !exists<DisableReconfiguration>(@open_libra)
    }
    ...
    public(friend) fun reconfigure() acquires Configuration {
        ...
        let config_ref = borrow_global_mut<Configuration>(@open_libra);
	...
	// 0L-TODO: New epoch reconfiguration.
    ...
    public fun last_reconfiguration_time(): u64 acquires Configuration {
        borrow_global<Configuration>(@open_libra).last_reconfiguration_time
    }

    public fun current_epoch(): u64 acquires Configuration {
        borrow_global<Configuration>(@open_libra).epoch
    }
    ...
    // This is used together with _::end_epoch() for testing with last_reconfiguration_time
