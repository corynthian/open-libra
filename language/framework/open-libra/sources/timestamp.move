...
module open_libra::timestamp {
    ...
    public(friend) fun set_time_has_started(open_libra: &signer) {
        system_addresses::assert_open_libra(open_libra);
        let timer = CurrentTimeMicroseconds { microseconds: 0 };
        move_to(open_libra, timer);
    }
    ...
    public fun update_global_time(
        ...
        // Can only be invoked by OLVM signer.
        system_addresses::assert_vm(account);

        let global_timer = borrow_global_mut<CurrentTimeMicroseconds>(@open_libra);
    ...
    public fun set_time_has_started_for_testing(account: &signer) {
        if (!exists<CurrentTimeMicroseconds>(@open_libra)) {
    ...
    public fun now_microseconds(): u64 acquires CurrentTimeMicroseconds {
        borrow_global<CurrentTimeMicroseconds>(@open_libra).microseconds
    }
