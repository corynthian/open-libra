module open_libra::state_storage {
    ...
    public(friend) fun initialize(open_libra: &signer) {
        ...
        assert!(
            !exists<StateStorageUsage>(@open_libra),
            error::already_exists(ESTATE_STORAGE_USAGE)
        );
        move_to(open_libra, StateStorageUsage {
            epoch: 0,
            usage: Usage {
                items: 0,
                bytes: 0,
    ...
    public(friend) fun on_new_block(epoch: u64) acquires StateStorageUsage {
        ...
        let usage = borrow_global_mut<StateStorageUsage>(@open_libra);
    ...
    public(friend) fun current_items_and_bytes(): (u64, u64) acquires StateStorageUsage {
        ...
        let usage = borrow_global<StateStorageUsage>(@open_libra);
