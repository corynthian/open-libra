...
module open_libra::storage_gas {
    ...
    public(friend) fun set_config(open_libra: &signer, config: StorageGasConfig) acquires StorageGasConfig {
        system_addresses::assert_open_libra(open_libra);
        *borrow_global_mut<StorageGasConfig>(@open_libra) = config;
    }
    ...
    /// target utilization.
    public fun initialize(open_libra: &signer) {
        system_addresses::assert_open_libra(open_libra);
        assert!(
            !exists<StorageGasConfig>(@open_libra),
        ...
        move_to(open_libra, StorageGasConfig {
            item_config,
            byte_config,
        });

        assert!(
            !exists<StorageGas>(@open_libra),
            error::already_exists(ESTORAGE_GAS)
        );
        move_to(open_libra, StorageGas {
            per_item_read: 300 * k,
            per_item_create: 5 * m,
    ...
    public(friend) fun on_reconfig() acquires StorageGas, StorageGasConfig {
        ...
        let gas_config = borrow_global<StorageGasConfig>(@open_libra);
        let gas = borrow_global_mut<StorageGas>(@open_libra);
    ...
    fun test_initialize_and_reconfig(framework: signer) acquires StorageGas, StorageGasConfig {
        ...
        on_reconfig();
        let gas_parameter = borrow_global<StorageGas>(@open_libra);
