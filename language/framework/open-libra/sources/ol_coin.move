...
module open_libra::ol_coin {
    ...
    use open_libra::coin::{Self, BurnCapability, MintCapability};
    use open_libra::system_addresses;

    friend open_libra::genesis;

    // 0L-TODO: Delegations might not be required (review)
    ...
    public(friend) fun initialize(open_libra: &signer): (BurnCapability<OLCoin>, MintCapability<OLCoin>) {
    ...
    public(friend) fun configure_accounts_for_test(
        ...
        // Mint the core resource account OLCoin for gas so it can execute system transactions.
        coin::register<OLCoin>(core_resources);
        let coins = coin::mint<OLCoin>(
            18446744073709551615,
            &mint_cap,
        );
        coin::deposit<OLCoin>(signer::address_of(core_resources), coins);
    ...
    public entry fun mint(
        ...
        let coins_minted = coin::mint<OLCoin>(amount, mint_cap);
        coin::deposit<OLCoin>(dst_addr, coins_minted);
    }
    ...
    #[test_only]
    public fun initialize_for_test(open_libra: &signer): (BurnCapability<OLCoin>, MintCapability<OLCoin>) {
        aggregator_factory::initialize_aggregator_factory_for_test(open_libra);
        initialize(open_libra)
    }

    // This is particularly useful if the aggregator_factory is already initialized via another call path.
    #[test_only]
    public fun initialize_for_test_without_aggregator_factory(open_libra: &signer): (BurnCapability<OLCoin>, MintCapability<OLCoin>) {
        initialize(open_libra)
    }
}
