...
use ol_vm::{
    ...
    OLVM, VMExecutor, VMValidator,
};
use ol_vm_genesis::{generate_genesis_change_set_for_testing_with_count, GenesisOptions};
use move_core_types::{
...
///
/// This struct is a mock in-memory implementation of the 0L executor.
#[derive(Debug)]
...
impl FakeExecutor {
    ...
    pub fn from_genesis(write_set: &WriteSet, chain_id: ChainId) -> Self {
        ...
        ol_vm::ol_vm::allow_module_bundle_for_test();
        executor
    }
    ...
    fn set_tracing(&mut self, test_name: &str, file_name: String) {
        ...
        if let Some(env_trace_dir) = env::var_os(ENV_TRACE_DIR) {
            ...
            let mut name_file = OpenOptions::new()
                ...
                .unwrap();
            write!(name_file, "{}::{}", test_name, ol_version).unwrap();
            for sub_dir in &[
                TRACE_DIR_META,
                TRACE_DIR_DATA,
                TRACE_DIR_INPUT,
    ...
    pub fn custom_genesis(framework: &ReleaseBundle, validator_accounts: Option<usize>) -> Self {
        let genesis = ol_vm_genesis::generate_test_genesis(framework, validator_accounts);
    ...
    pub fn new_account_at(&mut self, addr: AccountAddress) -> Account {
        ...
        // Mint the account 10M 0L coins (with 8 decimals).
    ...
    pub fn execute_transaction_block_parallel(
        ...
        BlockOLVM::execute_block(txn_block, &self.data_store, usize::min(4, num_cpus::get()))
    }
    ...
    pub fn execute_transaction_block(
        ...
        let output = OLVM::execute_block(txn_block.clone(), &self.data_store);
    ...
    pub fn verify_transaction(&self, txn: SignedTransaction) -> VMValidatorResult {
        let vm = OLVM::new(self.get_state_view());
