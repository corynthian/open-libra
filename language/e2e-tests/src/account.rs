...
//! Test infrastructure for modeling OL accounts.

use crate::gas_costs;
use ol_crypto::ed25519::*;
...
impl Account {
    ...
    /// not automatically get added to the OL store. To add an account to the store, use
    ...
    pub fn with_keypair(privkey: Ed25519PrivateKey, pubkey: Ed25519PublicKey) -> Self {
        let addr = ol_types::account_address::from_public_key(&pubkey);
        Account {
            addr,
            privkey,
            pubkey,
    ...
    /// Creates a new account representing the ol root account in memory.
    ///
    /// The address will be [`ol_test_root_address`][account_config::ol_test_root_address], and
    /// the account will use [`GENESIS_KEYPAIR`][struct@GENESIS_KEYPAIR] as its keypair.
    pub fn new_ol_root() -> Self {
        Self::new_genesis_account(account_config::ol_test_root_address())
    }
