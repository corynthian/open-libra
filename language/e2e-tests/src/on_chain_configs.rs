...
use ol_types::{account_config::CORE_CODE_ADDRESS, on_chain_config::Version};
use ol_vm::OLVM;

pub fn set_ol_version(executor: &mut FakeExecutor, version: Version) {
    let account = Account::new_genesis_account(CORE_CODE_ADDRESS);
    let txn = account
        .transaction()
        .payload(open_libra::version_set_version(version.major))
        .sequence_number(0)
        .sign();
    executor.new_block();
