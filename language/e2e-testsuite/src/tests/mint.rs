...
use ol_language_e2e_tests::{account::Account, executor::FakeExecutor};
use ol_types::transaction::{ExecutionStatus, TransactionStatus};

#[test]
fn mint_to_new_account() {
    let mut executor = FakeExecutor::from_head_genesis();
    let mut root = Account::new_ol_root();
    let (private_key, public_key) = ol_vm_genesis::GENESIS_KEYPAIR.clone();
    ...
    let txn = root
        .transaction()
        .payload(open_libra::ol_coin_mint(
            *new_account.address(),
            mint_amount,
        ))
        .sequence_number(0)
