...
use ol_framework::{ReleaseBundle, ReleasePackage};
use ol_gas::{
    AbstractValueSizeGasParameters, OLGasParameters, ChangeSetConfigs, InitialGasSchedule,
    NativeGasParameters, ToOnChainGasSchedule, LATEST_GAS_FEATURE_VERSION,
};
use ol_types::{
    account_config::{self, ol_test_root_address, events::NewEpochEvent, CORE_CODE_ADDRESS},
    chain_id::ChainId,
    contract_event::ContractEvent,
    on_chain_config::{
        FeatureFlag, Features, GasScheduleV2, OnChainConsensusConfig, OL_MAX_KNOWN_VERSION,
    },
...
const GOVERNANCE_MODULE_NAME: &str = "governance";
...
const OL_COINS_BASE_WITH_DECIMALS: u64 = u64::pow(10, 8);
...
pub fn default_gas_schedule() -> GasScheduleV2 {
    GasScheduleV2 {
        feature_version: ol_gas::LATEST_GAS_FEATURE_VERSION,
        entries: OLGasParameters::initial().to_on_chain_gas_schedule(LATEST_GAS_FEATURE_VERSION),
    }
}

pub fn encode_ol_mainnet_genesis_transaction(
...
pub fn encode_genesis_change_set(
    ...
    if genesis_config.is_test {
        initialize_core_resources_and_ol_coin(&mut session, core_resources_key);
    } else {
        initialize_ol_coin(&mut session);
    }
    initialize_on_chain_governance(&mut session, genesis_config);
...
fn initialize(
    ...
    exec_function(
        ...
        serialize_values(&vec![
            ...
            MoveValue::U64(OL_MAX_KNOWN_VERSION.major),
...
fn initialize_features(session: &mut SessionExt<impl MoveResolver>) {
    let features: Vec<u64> = vec![
        ...
        FeatureFlag::STD_CHAIN_ID_NATIVES as u64,
...
fn initialize_ol_coin(session: &mut SessionExt<impl MoveResolver>) {
    exec_function(
        session,
        GENESIS_MODULE_NAME,
        "initialize_ol_coin",
...
fn initialize_core_resources_and_ol_coin(
    session: &mut SessionExt<impl MoveResolver>,
    core_resources_key: &Ed25519PublicKey,
) {
    ...
    exec_function(
        session,
        GENESIS_MODULE_NAME,
        "initialize_core_resources_and_ol_coin",
        vec![],
        serialize_values(&vec![
...
fn allow_core_resources_to_set_version(session: &mut SessionExt<impl MoveResolver>) {
    exec_function(
        ...
        "initialize_for_test",
        vec![],
        serialize_values(&vec![MoveValue::Signer(ol_test_root_address())]),
    );
}
...
pub fn generate_genesis_change_set_for_testing_with_count(
    ...
    let framework = match genesis_options {
        GenesisOptions::Head => ol_cached_packages::head_release_bundle(),
        GenesisOptions::Testnet => ol_framework::testnet_release_bundle(),
        GenesisOptions::Mainnet => {
            // We don't yet have mainnet, so returning testnet here
            ol_framework::testnet_release_bundle()
        },
    };
...
pub fn generate_genesis_change_set_for_mainnet(genesis_options: GenesisOptions) -> ChangeSet {
    let framework = match genesis_options {
        ...
        GenesisOptions::Testnet => ol_framework::testnet_release_bundle(),
        // We don't yet have mainnet, so returning testnet here
        GenesisOptions::Mainnet => ol_framework::testnet_release_bundle(),
    };
...
pub fn test_genesis_change_set_and_validators(
    ...
    generate_test_genesis(ol_cached_packages::head_release_bundle(), count)
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Validator {
    /// The 0L account address of the validator or the admin in the case of a commissioned or
    /// vesting managed validator.
    pub owner_address: AccountAddress,
    /// The 0L account address of the validator's operator (same as `address` if the validator is
    /// its own operator).
...
fn mainnet_genesis_config() -> GenesisConfiguration {
    ...
    GenesisConfiguration {
        ...
        is_test: false,
        min_stake: 1_000_000 * OL_COINS_BASE_WITH_DECIMALS, // 1M APT
        // 400M APT
        min_voting_threshold: (400_000_000 * OL_COINS_BASE_WITH_DECIMALS as u128),
        max_stake: 50_000_000 * OL_COINS_BASE_WITH_DECIMALS, // 50M APT.
        recurring_lockup_duration_secs: 30 * 24 * 3600,         // 1 month
        required_proposer_stake: 1_000_000 * OL_COINS_BASE_WITH_DECIMALS, // 1M APT
        rewards_apy_percentage: 10,
...
pub fn test_genesis_module_publishing() {
    ...
    for (module_bytes, module) in
        ol_cached_packages::head_release_bundle().code_and_compiled_modules()
    {
        state_view.add_module(&module.self_id(), module_bytes);
    }
    let data_cache = state_view.as_move_resolver();
    ...
    let id1 = HashValue::zero();
    let mut session = move_vm.new_session(&data_cache, SessionId::genesis(id1));
    publish_framework(&mut session, ol_cached_packages::head_release_bundle());
}
...
pub fn test_mainnet_end_to_end() {
    ...
    let balance = 10_000_000 * OL_COINS_BASE_WITH_DECIMALS;
    let non_validator_balance = 10 * OL_COINS_BASE_WITH_DECIMALS;
    ...
    let transaction = encode_ol_mainnet_genesis_transaction(
        &accounts,
        &employees,
        &validators,
        ol_cached_packages::head_release_bundle(),
        ChainId::mainnet(),
        &mainnet_genesis_config(),
    );
