...
//!
//! '''bash
//! cargo run -p ol-transaction-builder -- --help
//! '''
...
#[derive(Debug, StructOpt)]
#[structopt(name = "0L SDK Builder", about = "Generate boilerplate 0L SDKs")]
struct Options {
    ...
    /// Also install the ol types described by the given YAML file, along with the BCS runtime.
    #[structopt(long)]
    with_ol_types: Option<PathBuf>,
    ...
    /// and `ol_types` is assumed to be in "go_module/path/ol_types".
    #[structopt(long)]
    ...
    /// Optional version number for the `ol_types` module (useful in Rust).
    /// If `--with-ol-types` is passed, this will be the version of the generated `ol_types` module.
    #[structopt(long, default_value = "0.1.0")]
    ol_version_number: String,

    /// Optional package name (Python) or module path (Go) of the `ol_types` dependency.
    #[structopt(long)]
...
fn main() {
    ...
    let abis = ol_transaction_builder::read_abis(&options.abi_directories)
    ...
    let install_dir = match options.target_source_dir {
        None => {
            ...
            match options.language {
                Language::Rust => {
                    ol_transaction_builder::rust::output(&mut out, &abis, /* local types */ true)
                        .unwrap()
                },
                Language::Go => {
                    ol_transaction_builder::golang::output(
                        &mut out,
    ...
    // 0L types
    if let Some(registry_file) = options.with_ol_types {
        ...
        if let Language::Rust = options.language {
            ol_transaction_builder::rust::replace_keywords(&mut registry)
        }

        let (package_name, _package_path) = match options.language {
            Language::Rust => (
                if options.ol_version_number == "0.1.0" {
                    "ol-types".to_string()
                } else {
                    format!("ol-types:{}", options.ol_version_number)
                },
                vec!["ol-types"],
            ),
            Language::Go => ("oltypes".to_string(), vec!["oltypes"]),
        };
    ...
    let installer: Box<dyn ol_transaction_builder::SourceInstaller<Error = Box<dyn std::error::Error>>> =
        match options.language {
            Language::Rust => Box::new(ol_transaction_builder::rust::Installer::new(
                install_dir,
                options.ol_version_number,
            )),
            Language::Go => Box::new(ol_transaction_builder::golang::Installer::new(
                install_dir,
