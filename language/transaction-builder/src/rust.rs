...
/// If `local_types` is true, we generate a file suitable for the 0L codebase itself
...
struct RustEmitter<T> {
    ...
    /// Whether we are targetting the 0L repository itself (as opposed to generated 0L types).
    local_types: bool,
}
    ...
    fn get_external_definitions(local_types: bool) -> serde_generate::ExternalDefinitions {
        let definitions = if local_types {
            vec![
                ...
                ("ol_types::transaction", vec![
                    "TransactionPayload",
                    "EntryFunction",
                ]),
                ("ol_types::account_address", vec!["AccountAddress"]),
            ]
        } else {
            vec![("ol_types", vec![
                "AccountAddress",
...
pub fn decode(payload: &TransactionPayload) -> Option<EntryFunctionCall> {{
    ...
    fn output_transaction_script_name_method(
        ...
        writeln!(
            self.out,
            r#"
/// Return the name of an 0L `Script` from a structured object `ScriptCall`.
...
pub struct Installer {
    install_dir: PathBuf,
    ol_types_version: String,
}

impl Installer {
    pub fn new(install_dir: PathBuf, ol_types_version: String) -> Self {
        Installer {
            install_dir,
            ol_types_version,
        }
...
serde_bytes = "0.11.6"
ol-types = {{ path = "../ol-types", version = "{}" }}
"#,
            name, version, self.ol_types_version,
        )?;
