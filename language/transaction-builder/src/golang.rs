        ...
        .filter(|abi| {
            if let EntryABI::EntryFunction(sf) = abi {
                sf.module_name().name().as_str() != "code"
                    ...
                    && sf.module_name().name().as_str() != "ol_account"
    ...
    T: Write,
{
    fn output_script_call_enum_with_imports(&mut self, abis: &[EntryABI]) -> Result<()> {
        let ol_types_package = match &self.ol_module_path {
            Some(path) => format!("{}/oltypes", path),
            None => "oltypes".into(),
        };
        let mut external_definitions =
            crate::common::get_external_definitions(&ol_types_package);
        // We need BCS for argument encoding and decoding
        external_definitions.insert(
            "github.com/corynthian/serde-reflection/serde-generate/runtime/golang/bcs".to_string(),
            Vec::new(),
        );
    ...
    fn output_encode_method(&mut self, abis: &[EntryABI]) -> Result<()> {
        ...
        if !transaction_script_abis.is_empty() {
            writeln!(
                self.out,
                r#"
// Build an 0L `Script` from a structured object `ScriptCall`.
func EncodeScript(call ScriptCall) oltypes.Script {{"#
            )?;
            self.out.indent();
...
func EncodeEntryFunction(call EntryFunctionCall) oltypes.TransactionPayload {{"#
            )?;
            self.out.indent();
            writeln!(self.out, "switch call := call.(type) {{")?;
    ...
    fn output_transaction_script_decode_method(&mut self) -> Result<()> {
        writeln!(
            self.out,
            r#"
// Try to recognize an 0L `Script` and convert it into a structured object `ScriptCall`.
func DecodeScript(script *oltypes.Script) (ScriptCall, error) {{
...
func DecodeEntryFunctionPayload(script oltypes.TransactionPayload) (EntryFunctionCall, error) {{
    switch script := script.(type) {{
        case *oltypes.TransactionPayload__EntryFunction:
    ...
    fn output_transaction_script_encoder_function(
        ...
        writeln!(
            self.out,
            r#"return oltypes.Script {{
	Code: append([]byte(nil), {}_code...),
	TyArgs: []oltypes.TypeTag{{{}}},
	Args: []oltypes.TransactionArgument{{{}}},
}}"#,
            abi.name(),
    ...
    fn output_entry_function_encoder_function(&mut self, abi: &EntryFunctionABI) -> Result<()> {
        writeln!(
            self.out,
            "\n{}\nfunc Encode{}{}({}) oltypes.TransactionPayload {{",
            Self::quote_doc(abi.doc()),
            abi.module_name().name().to_string().to_camel_case(),
            abi.name().to_camel_case(),
            [
        ...
        writeln!(
            self.out,
            r#"return &oltypes.TransactionPayload__EntryFunction {{
            oltypes.EntryFunction {{
                Module: {},
                Function: {},
                TyArgs: []oltypes.TypeTag{{{}}},
                Args: [][]byte{{{}}},
    ...
    fn output_transaction_script_decoder_function(
        ...
        writeln!(
            self.out,
            "\nfunc decode_{}(script *oltypes.Script) (ScriptCall, error) {{",
            abi.name(),
        )?;
        self.out.indent();
        writeln!(
...
"#,
    ...
    fn output_entry_function_decoder_function(&mut self, abi: &EntryFunctionABI) -> Result<()> {
        writeln!(
            self.out,
            "\nfunc decode_{}_{}(script oltypes.TransactionPayload) (EntryFunctionCall, error) {{",
...
"#,
    ...
    fn output_transaction_script_decoder_map(
        ...
        writeln!(
            self.out,
            r#"
var script_decoder_map = map[string]func(*oltypes.Script) (ScriptCall, error) {{"#
        )?;
        self.out.indent();
        for abi in abis {
    ...
    fn output_entry_function_decoder_map(&mut self, abis: &[EntryFunctionABI]) -> Result<()> {
        writeln!(
            self.out,
            r#"
var entry_function_decoder_map = map[string]func(oltypes.TransactionPayload) (EntryFunctionCall, error) {{"#
        )?;
        self.out.indent();
        for abi in abis {
            writeln!(
...
"#,
    ...
    fn output_decoding_helper(&mut self, type_tag: &TypeTag) -> Result<()> {
        ...
        writeln!(
            self.out,
            r#"
func decode_{0}_argument(arg oltypes.TransactionArgument) (value {1}, err error) {{
	if arg, ok := arg.(*oltypes.TransactionArgument__{2}); ok {{
		{3}
	}} else {{
...
"#,
    ...
    fn quote_type_parameters(ty_args: &[TypeArgumentABI]) -> Vec<String> {
        ty_args
            .iter()
            .map(|ty_arg| format!("{} oltypes.TypeTag", ty_arg.name()))
            .collect()
    }
    ...
    fn quote_type(type_tag: &TypeTag) -> String {
        ...
        match type_tag {
            ...
            U256 => unimplemented!(),
            Address => "oltypes.AccountAddress".into(),
            Vector(type_tag) => {
    ...
    fn quote_transaction_argument_for_script(type_tag: &TypeTag, name: &str) -> String {
        use TypeTag::*;
        match type_tag {
            Bool => format!("(*oltypes.TransactionArgument__Bool)(&{})", name),
            U8 => format!("(*oltypes.TransactionArgument__U8)(&{})", name),
            U16 => format!("(*oltypes.TransactionArgument__U16)(&{})", name),
            U32 => format!("(*oltypes.TransactionArgument__U32)(&{})", name),
            U64 => format!("(*oltypes.TransactionArgument__U64)(&{})", name),
            U128 => format!("(*oltypes.TransactionArgument__U128)(&{})", name),
            U256 => format!("(*oltypes.TransactionArgument__U256)(&{})", name),
            Address => format!("&oltypes.TransactionArgument__Address{{{}}}", name),
            Vector(type_tag) => match type_tag.as_ref() {
                U8 => format!("(*oltypes.TransactionArgument__U8Vector)(&{})", name),
                _ => common::type_not_allowed(type_tag),
            },
            Struct(_) | Signer => common::type_not_allowed(type_tag),
        }
