...
use ol_crypto_derive::{BCSCryptoHash, CryptoHasher};
...
pub struct RawTransaction {
    ...
    /// Chain ID of the Ol network this transaction is intended for.
    chain_id: ChainId,
}
...
/// `Transaction` will be the transaction type used internally in the ol node to represent the
...
pub enum Transaction {
    ...
    /// Transaction that applies a WriteSet to the current storage, it's applied manually via ol-db-bootstrapper.
