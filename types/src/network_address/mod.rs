...
/// ## Overview
///
/// The open libra `NetworkAddress` is a compact, efficient, self-describing and
...
/// 4. Perform a 0LNet version negotiation handshake (version 1).
///
...
/// ## Transport
///
/// In addition, `NetworkAddress` is integrated with the 0LNet concept of a
...
///
/// use ol_types::network_address::NetworkAddress;
/// use bcs;
...
impl NetworkAddress {
    ...
    ///
    /// ```rust
    /// use ol_crypto::{traits::ValidCryptoMaterialStringExt, x25519};
    /// use ol_types::network_address::NetworkAddress;
    /// use std::str::FromStr;
    ///
    ...
    /// Check that a `NetworkAddress` looks like a typical 0LNet address with
    /// associated protocols.
    ///
    /// "typical" 0LNet addresses begin with a transport protocol:
    ///
    ...
    /// assert!(addr.is_olnet_addr());
    /// ```
    pub fn is_olnet_addr(&self) -> bool {
        parse_olnet_protos(self.as_slice()).is_some()
    }
...
#[cfg(any(test, feature = "fuzzing"))]
pub fn arb_olnet_addr() -> impl Strategy<Value = NetworkAddress> {
    ...
    let arb_olnet_protos = any::<(x25519::PublicKey, u8)>()
        .prop_map(|(pubkey, hs)| vec![Protocol::NoiseIK(pubkey), Protocol::Handshake(hs)]);

    (arb_transport_protos, arb_olnet_protos).prop_map(
        |(mut transport_protos, mut olnet_protos)| {
            transport_protos.append(&mut olnet_protos);
...
/// parse canonical olnet protocols
///
/// See: [`NetworkAddress::is_olnet_addr`]
fn parse_olnet_protos(protos: &[Protocol]) -> Option<&[Protocol]> {
...
mod test {
    ...
    proptest! {
        #[test]
        fn test_network_address_canonical_serialization(addr in arb_olnet_addr()) {
        ...
        #[test]
        fn test_network_address_display_roundtrip(addr in arb_olnet_addr()) {
        ...
        #[test]
        fn test_is_olnet_addr(addr in arb_olnet_addr()) {
            assert!(addr.is_olnet_addr(), "addr.is_olnet_addr() = false; addr: '{}'", addr);
        }
    }
}
